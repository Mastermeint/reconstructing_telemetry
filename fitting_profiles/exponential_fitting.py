...
customer = pd.read_csv('../Alliander_data/unique_meetdata.csv', index_col = 0, nrows = 200)
...
#Here is the fourth method that I tried, unfortunately gives overflow

def optimal_parameters_4(profile_df, customer_df):
    ...
    x0 = [1,1]
    opt = minimize(equation,x0) 
    res = opt.x
    
    return res

#Here is the fifth method that I tried
from scipy import optimize

def optimal_parameters_5(profile_np, customer_np):
    
    def equations(p):
        a, b = p
        return [np.sum(a*np.sum(customer_np)*profile_np**b) - np.sum(customer_np), np.amax(a*np.sum(customer_np)*profile_np**b) - np.amax(customer_np)]

    opt =  optimize.root(equations, [1, 1])

    res = opt.x
    print(equations((res[0], res[1])))
    
    return res


'''
   ATTENTION: after experimenting with the five methods turns out that the only
   two that are working are the first one and the fifth one, were we try to solve the system numerically
   starting with (1,1) point. However, after trying out many other profiles as well,
   turns out that such method converges to a useful solution only few times.
   ...
   such that there is no worse result in any case.
