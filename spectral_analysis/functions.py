...
def compute_df_frequencies_1(df, number_customers_df):
    diff_meetdata_pd = produce_differences_df(df)
    number_customers_np = number_customers_df.to_numpy()
    ...
    for i in range(shape[0]):
        ...
        if condition:
            peak = compute_frequency_peak(diff_company_i)
            estimated_peaks.append([number_customers_np[i], peak])
        else:
            estimated_peaks.append([number_customers_np[i], 0])
